...
# No fagent CLI on PATH → this isn't a fleet-managed environment; do nothing.
command -v fagent >/dev/null 2>&1 || exit 0

# `fagent agent in-workspace` prints `repo/name` and exits 0 inside a workspace,
...
workspace="$(fagent agent in-workspace 2>/dev/null)" || exit 0
