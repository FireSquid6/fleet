  ...
  return (
    <header className="flex h-[53px] flex-none items-center gap-2 border-b border-line bg-panel px-4 md:justify-between md:px-[22px]">
      ...
      <div className="flex flex-none items-center gap-2">
        ...
        {user && (
          <>
            <span className="hidden font-mono text-[10.5px] font-medium text-dim sm:inline">
              {user.username}
            </span>
            <button
              type="button"
              onClick={() => void signOut()}
              className="rounded-[3px] border border-line px-[11px] py-1.5 font-mono text-[10.5px] font-medium text-dim transition-colors hover:bg-panel2 hover:text-text"
            >
              sign out
            </button>
          </>
